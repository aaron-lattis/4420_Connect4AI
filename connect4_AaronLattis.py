...
###############################################################################
    ...
    def minimax2(node, depth, maximizing, player):

        if (depth == 0 or node.terminal()):

            return evaluate(player, node), None
        ...
        if (maximizing):
            ...
            newBoards = get_child_boards(player, node)

            for board in newBoards:

                moveValueTuple = minimax2(board[1], depth - 1, False, player)

                moveValue = moveValueTuple[0]
            ...
            if (player == node.PLAYER1):

                newBoards = get_child_boards(node.PLAYER2, node)

            else:

                newBoards = get_child_boards(node.PLAYER1, node)

            for board in newBoards:

                moveValueTuple = minimax2(board[1], depth - 1, True, player)
    ...
    placementTuple = minimax2(board, depth_limit, True, player)

    placement = placementTuple[1]
...
###############################################################################
    ...
    def prune(node, depth, maximizing, player, alpha, beta):
        ...
        if (maximizing):
            ...
            for board in newBoards:
                ...
                moveValueTuple = prune(board[1], depth - 1, False, player, alpha, beta)
            ...
            for board in newBoards:
                ...
                moveValueTuple = prune(board[1], depth - 1, True, player, alpha, beta)
    ...
    placementTuple = prune(board, depth_limit, True, player, -100000, 100000)
...
###############################################################################
    ...
    def expecti(node, depth, maximizing, player):
        ...
        if (maximizing):
            ...
            for board in newBoards:
                ...
                moveValueTuple = expecti(board[1], depth - 1, False, player)
            ...
            for board in newBoards:
                ...
                moveValueTuple = expecti(board[1], depth - 1, True, player)

                avg += moveValueTuple[0]

            avg = avg / node.cols

            return avg, None

    placementTuple = expecti(board, depth_limit, True, player)
